...
```javascript
// Location: vibe-to-docker.js (lines 297-417)
...
│   └── assets/
│
├── .vibe-docker/  (generated by vibe-to-docker)
│   ├── Dockerfile
│   ├── docker-compose.yml
│   ├── nginx.conf
│   ├── .env.example
...
#### Detection Location
Lines 230-235 in vibe-to-docker.js:
...
## 7. .vibe-docker Directory Structure

### 7.1 Generated Directory Layout

Created by vibe-to-docker when run on Figma Make projects:
...
```javascript
// From vibe-to-docker.js lines 441-451
const requiredVars = [
...
This research was conducted during the planned migration from **vibe-to-docker** to **vibe-to-docker**, a more generalized tool supporting multiple vibe-coding platforms:
...
4. Preserve per-project `.vibe-docker/` structure (replacing `.vibe-docker/`)
...
From test/vibe-to-docker.test.js (lines 46-56):
...
- Maintain backward compatibility with `.vibe-docker/` directory structure
...
- Supporting seamless migration from legacy vibe-to-docker
